...
public class HtmlParser {
	...
	Seller parseSeller(Element row) {
		...
		if (this.getText(row, sellerNameSelector).equals("")){
			seller.setName("AP");
			seller.setUuid("0");
			seller.setRating(100);
			seller.setRatingCount(1000);
		}else{
			seller.setName(this.getText(row, sellerNameSelector));
//			String link = row.select(sellerNameSelector + "a").get(0).attr("href");
			String link = row.select(sellerNameSelector).tagName("a").get(0).attr("href");
		...
		for (Element element : deliveries) {
			...
			} else if (text.contains("International & domestic shipping ratesand return policy.")) {
			...
			} else if (text.contains("United Kingdom")){
